---
title: "お気に入りのお弁当屋さんの日替わり弁当を通知する Line Bot を作った"
emoji: "👋"
type: "tech" # tech: 技術記事 / idea: アイデア
topics: ["LINE Bot", "LINE", "Twitter", "API", "GCP"]
published: false
---
...
難しいことは何もしておらず、お弁当屋さんが毎朝ツイートする内容を昼前に LINE へ転送するシンプルな Bot です。
...
1年以上週1くらいの頻度で通ってますが、日替わりメニューが被ったことがほとんどありません。

訪れるたびに変わる日替わりが楽しみな半面、行けない日に美味しそうな日替わりが出ているとちょっと損した気分になります。
そこで、日替わり弁当のメニューを毎日通知してくれるサービスを作り、日替わりの内容を常に把握できるような仕組みを作ってみました。

## 成果物

毎日お昼の 11 時に LINE メッセージを送る LINE Bot を作成しました。
メッセージ内容は、お弁当屋さんの Tweet 内容を流用しています。

TODO: スクリーンショットの貼り付け

## 要件定義

日替わり弁当のメニューを通知するためには、メニュー情報を取得する必要があります。
幸いに、お弁当屋さんは Twitter を利用しており、毎日朝 8 時ごろに日替わり弁当のメニューをツイートしています。

それを見れば十分ですね、今回の開発は終了です、、、とはならないのがシステム開発です。
わがままなユーザー(私)は Twitter をやっていません。
日替わりメニューを見るだけのために Twitter をインストールしたくないという、とてもめんどくさいユーザーでした。
...
まず、言語は Python を使用することにします。
理由は、サードパティ製の Twitter API Wrapper `tweepy` と公式の LINE BOT SDK どちらも一度使ったことがあるからです。

次にデプロイ先ですが、今回はシンプルな機能なのでサーバーレス関数として実装できそうです。
また毎日日替わり弁当の通知をするため、定期的な自動実行が必要です。
メジャーなサービスならどれでも実現できますが、個人的な好みで GCP の Cloud Functions を採用します。

ということで、最終的に以下画像のような形で実装しました。
...
3. ページ最下部の 「+ Create Aoo」をクリック

![Show where click to create app](/images/dairy-bento/CreateStandaloneApps.png)
...
![Show generate access token button](/images/dairy-bento/GetAccessToken.png)
...
お弁当屋さんは毎朝の日替わりツイートしかしないので、5 件だけ取得しています。
